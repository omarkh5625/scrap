...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            // Generate secure configuration file with proper escaping
            // Using json_encode for safe escaping, then convert to PHP string
            $safeHost = addslashes($dbHost);
            $safePort = addslashes($dbPort);
            ...
            $configContent .= " */\n\n";
            $configContent .= "define('DB_HOST', '{$safeHost}');\n";
            $configContent .= "define('DB_PORT', '{$safePort}');\n";
            $configContent .= "define('DB_USER', '{$safeUser}');\n";
            $configContent .= "define('DB_PASS', '{$safePass}');\n";
            $configContent .= "define('DB_NAME', '{$safeName}');\n";
